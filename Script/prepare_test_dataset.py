...
def assign_domain(output_text):
    ...
    """
    if "Severity of Symptoms and Mortality" in output_text:
        return "Clinical_impact"
    elif "Political & Social Factors" in output_text:
        return "External_risk"
    elif "Population Density" in output_text:
        return "Geographic"
    elif "Availability of Treatments and Vaccines" in output_text:
        return "Healthcare_system"
    elif "Pathogen Identification" in output_text:
        return "Pathogen"
    elif "Mode of Transmission" in output_text:
        return "Transmission"
    else:
        return "Unknown"
...
def extract_signal_start(input_text):
    ...
    signal_in_english = re.split(r"Signal in English:", input_text)
...
def extract_article_content(input_text):
    ...
    """
    parts = input_text.split("Signal in English:")
    if len(parts) > 1:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path", 
        default="./Data/Merged/Severity_data/test.json",
        help="Path to the input test.json file"
    )
    parser.add_argument(
        "--output_path", 
        default="./Data/Processed/Severity_data/test.json",
